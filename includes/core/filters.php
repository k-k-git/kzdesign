...
function barcelona_password_form() {
	...
	<p class="description">' . esc_html__( 'このページはパスワードによって保護されています。ご閲覧を希望のお客様はパスワードの入力が必要になりますので、お問い合わせフォームにてご連絡下さい。', 'barcelona' ) . '</p>
	<p class="input-group"><input name="post_password" id="' . esc_attr( $label ) . '" class="form-control" type="password" size="20" placeholder="'. esc_html__( 'パスワードを入力して下さい。', 'barcelona' ) .'" /><span class="input-group-btn"><button type="submit" class="btn btn-red-2" name="submit">'. esc_attr__( '送信', 'barcelona' ) .'</button></span></p></form>';
