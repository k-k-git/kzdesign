	...
	<div <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
		<article id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
			<div class="comment-author vcard">
					<?php echo gtcn_comment_numbering($comment->comment_ID, $args); ?><span class="kugiri">:</span>
				<cite class="fn">
				...
				<span style="margin-left: 10px" class="cm-id">ID：
	                <?php //IDっぽい文字列の表示（あくまでIDっぽいものです。）
						...
						<div class="vm-wrapper">
							<div class="vm-middle">
								<?php echo ( $barcelona_display == 'title' ? '' : $barcelona_media_output ) . $barcelona_post_title ."\n". $barcelona_post_meta; ?>
										<?php if (get_the_modified_date('Y/n/j') != get_the_time('Y/n/j')) : ?><span class="final-update clearfix">最終更新日：<?php the_modified_date('Y年n月j日') ?></span><?php endif; ?>
...
function barcelona_related_posts() {
	...
	if ( $barcelona_retrieved_posts > 0 ) {
		...
		$barcelona_mod_header = '<div class="box-header archive-header has-title"><h2 class="title main-bg">'. esc_html__( '人気トピック', 'barcelona' ) .'</h2></div>';
