...
if ( barcelona_get_option( 'disqus_comments' ) != 'on' ) {
	...
	<div id="comments">
		...
		<h2 class="comments-title">
			<?php comments_number( esc_html__( 'No Comments', 'barcelona' ), esc_html__( 'この記事へのコメント', 'barcelona' ), esc_html( _n( '% Comment', 'この記事へのコメント  % 件', number_format_i18n( get_comments_number() ), 'barcelona' ) ) ); ?>
		</h2>
		...
		<span class="cm-info">※コメント反映までに時間を要する場合があります。</span>
		<?php
			...
			comment_form(
				array(
					'title_reply'           => esc_html__( '掲示板へつぶやく', 'barcelona' ),
					...
					'cancel_reply_link'     => esc_html__( '返信をキャンセル', 'barcelona' ),
					'label_submit'          => esc_html__( 'つぶやく', 'barcelona' ),
					...
					'comment_field'         => '<div class="col-sm-12"><div class="form-group comment-form-comment"><textarea style="height: 80px" id="comment" class="form-control" name="comment" rows="10" placeholder="'. esc_html_x( 'コメントを入力して下さい', 'noun', 'barcelona' ) .' *" required="required"></textarea></div></div>',
					...
					'format'                => 'html5',
					'fields' => array(
						'author'    => '<div class="col-sm-6"><div class="form-group comment-form-author"><input id="author" class="form-control" type="text" name="author" placeholder="'. esc_html__( '名無しのエンジニア', 'barcelona' ) . ( $barcelona_req ? ' *' : '' ) .'" value="' . esc_attr( $barcelona_commenter['comment_author'] ) . '" size="30"'. $barcelona_aria_req . $barcelona_html_req .'></div></div>'
					)
				)
			);
